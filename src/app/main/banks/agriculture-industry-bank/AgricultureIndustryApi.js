...
const AgricultureIndustryApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      getAllAgricultureIndustryCompanies: build.query({
        query: ({ pageNumber, pageSize, search, sort, filter }) => ({
          url: `/company/?categoryId=2&pageNumber=${pageNumber}&pageSize=${pageSize}&search=${search}&sort=${(sort && Object.entries(sort)?.length && JSON.stringify(sort)) || ''}&filter=${(filter && Object.entries(filter)?.length && JSON.stringify(filter)) || ''}`,
          method: 'GET'
        }),
      ...
      getAgricultureIndustryCompanyDetails: build.query({
        query: (companyId) => ({
          url: `/company/${companyId}`
        }),
        transformResponse: (response) => response?.data,
        providesTags: ['agricultureCompanyList', 'agricultureCompanyDetails']
      }),
      ...
      invalidatesTags: (result, error, { companyId }) => [
        { type: 'AgricultureIndustryCompany', id: companyId },
      ],
    }),
    ...
    createCompany: build.mutation({
        ...
        invalidatesTags: ['agricultureCompanyList']
      }),
      ...
      updateCompany: build.mutation({
        ...
        invalidatesTags: (result, error, { id }) => [
          { type: 'agricultureCompanyDetails', id },
          'agricultureCompanyList'
        ]
      }),
    saveCompanyGalleryFiles: build.mutation({
      query: ({ companyId, galleryFiles }) => ({
        url: `agriculture-industry-bank/companies/${companyId}/gallery-files`,
        method: 'POST',
      ...
      rollbackToArchive: build.mutation({
        ...
        invalidatesTags: ['companyArchives', 'agricultureCompanyDetails']
      }),
...
export const {
  ...
  useGetAllAgricultureIndustryCompanyRequestsQuery,
  useGetAgricultureIndustryCompanyDetailsQuery,

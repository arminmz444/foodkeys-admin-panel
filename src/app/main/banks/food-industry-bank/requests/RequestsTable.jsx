...
import { useGetAllFoodIndustryCompanyRequestsQuery } from '../FoodIndustryBankApi';
import {
...
function RequestsTable() {
	...
	const columns = React.useMemo(
		() => [
			...
			{
				accessorKey: 'companyPrimaryBrand',
				header: 'برند تجاری اصلی شرکت',
				size: 250,
				Cell: ({ row }) =>
					row.original.companyPrimaryBrand ? (
						<Typography
							component={row.original.companyId ? Link : 'p'}
							to={`/banks/food-industry/company/${row.original.companyId}`}
							className="underline"
							color="secondary"
							role="button"
						>
	...
	return (
		<div className="w-full flex flex-col">
			<FuseScrollbars className="grow overflow-x-auto">
				...
				<GenericCrudTable
					columns={columns}
					useListQueryHook={useGetAllFoodIndustryCompanyRequestsQuery}

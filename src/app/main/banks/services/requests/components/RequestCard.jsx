...
import { useGetServiceByIdQuery } from '../../ServicesBankApi';
...
const requestTypeMap = {
  UNKNOWN: 'نامشخص',
  CONSUMER_CREATE_SERVICE_REQUEST: 'درخواست ایجاد سرویس',
  CONSUMER_UPDATE_SERVICE_REQUEST: 'درخواست ویرایش سرویس',
  CONSUMER_DELETE_SERVICE_REQUEST: 'درخواست حذف سرویس',
...
export default function RequestCard({ request, onActionComplete }) {
  ...
  const { data: serviceDetails, isFetching: isServiceLoading } = useGetServiceByIdQuery(
    request?.service?.id || request?.serviceId,
    { 
  ...
  const service = serviceDetails || request?.service;
  ...
  const getStatusDetails = (status) => {
    return requestStatusMap[status] || requestStatusMap.UNKNOWN;
  };

  const getRequestTypeName = (type) => {
    return requestTypeMap[type] || requestTypeMap.UNKNOWN;
  };

  const statusDetails = getStatusDetails(request.requestStatus);
  const StatusIcon = statusDetails.icon;
  
  const logoUrl = (service?.logo && getServerFile(service?.logo)) || 'assets/images/placeholders/company-logo.png';
  
  const backgroundUrl = (service?.backgroundImage && getServerFile(service?.backgroundImage)) || 'assets/images/placeholders/company-banner.png';
      ...
      >
        ...
        <CardHeader
          avatar={
            <Avatar 
              ...
              alt={service?.name || request?.serviceName}
            />
          }
          action={
          ...
          title={
            <Tooltip title={service?.name || request?.serviceName || 'نامشخص'}>
        ...
        <Collapse in={expanded} timeout="auto" unmountOnExit>
          <CardContent>
            ...
            {(service?.subCategoryName || service?.ranking || service?.keyWords) && (
              <>
                <Typography variant="subtitle2" sx={{ mt: 2, mb: 1 }}>اطلاعات سرویس:</Typography>
                <div className="grid grid-cols-2 gap-16">
                  {service.subCategoryName && (
                    <>
                      <Typography variant="body2" color="text.secondary">زیردسته:</Typography>
                      <Typography variant="body2">{service.subCategoryName}</Typography>
                    </>
                  )}
                  
                  {service.ranking && (
                    <>
                      <Typography variant="body2" color="text.secondary">رتبه:</Typography>
                      <Typography variant="body2">{service.ranking}</Typography>
                    </>
                  )}
                  
                  {service.keyWords && (
                    <>
                      <Typography variant="body2" color="text.secondary">کلمات کلیدی:</Typography>
                      <Typography variant="body2">{service.keyWords}</Typography>
                    </>
                  )}
                </div>
              </>

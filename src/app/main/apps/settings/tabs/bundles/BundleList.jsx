...
function BundleList() {
    ...
    const handleBundleUpdated = () => {
        // Refetch both queries to ensure data consistency
        refetchBundles();
        refetchInfo();
    };

    const handleBundleAdded = () => {
        // Refetch both queries and show success message
        refetchBundles();
        refetchInfo();
        enqueueSnackbar("پلن با موفقیت اضافه شد", {variant: "success", style: {fontSize: 'medium'}});
    };
    ...
    return (
        <FusePageSimple
            content={
                <ContentWrapper>
                    <Hidden lgUp>
                        <Paper className="flex sticky top-0 z-10 items-center w-full px-16 py-8 border-b-1 shadow-0"
                               square>
                            <IconButton to="/apps/settings/bundle" component={Link}>
                                <FuseSvgIcon>
                                    {theme.direction === "ltr" ? "heroicons-outline:arrow-sm-left" : "heroicons-outline:arrow-sm-right"}
                        ...
                        {Array.from({length: totalPages}).map((_, index) => (
                            <div key={index}
                                ...
                                {isLoadingDetails ? (
                                        ...
                                        <div className="prose prose-sm dark:prose-invert w-full max-w-full"
                                            ...
                                            <BundleCardContainer>
                                                <BundleCard
                                                    ...
                                                    features={bundleDetails.features?.split(".")}
                                                    updatedAt={bundleDetails.updatedAtStr}
                                                    timeDuration={bundleDetails.duration}
                                                    status={bundleDetails.status}
                                                    providedAccess={bundleDetails.providedAccess}
                                    ...
                                    <Paper className="p-24 text-center">
                                        <Typography>اطلاعات پلن در دسترس نیست</Typography>
                                    </Paper>
                                )}
                            </div>
                        ))}
                    ...
                    {totalPages > 0 && (
                        <>
                            ...
                            <Hidden lgDown>
                                <div className="flex justify-center w-full sticky bottom-0 p-16 pb-32 z-10">
                                    <ButtonGroup variant="contained" aria-label="pagination" className="rounded-full"
                                                 color="secondary">
                                        <Button
                                            className="min-h-56 rounded-full"
                                            size="large"
                            ...
                            <Hidden lgUp>
                                <Box
                                    ...
                                    className="flex sticky bottom-0 z-10 items-center w-full p-16 border-t-1"
                                >
                                    <IconButton onClick={() => setLeftSidebarOpen(true)} aria-label="open left sidebar"
                                                size="large">
                                        <FuseSvgIcon>heroicons-outline:view-list</FuseSvgIcon>
                                    </IconButton>
                ...
                <>
                    <div className="p-32">
                        ...
                        <div>
                            ...
                            <div className="flex gap-5 ">
                                <IoMdTime size={20}/>
                                <Typography variant="caption">اخرین بروزرسانی
                                    : {subCategoryInfo?.updatedAt || "نامشخص"}</Typography>
                            </div>

                            <div className="flex gap-5 text-green-500 mt-8">
                                <RiVerifiedBadgeFill size={20}/>
                                <Typography variant="caption">تعداد پلن‌های فعال
                                    : {subCategoryInfo?.totalActive || 0}</Typography>
                            </div>
                            <div className="flex pt-32 justify-center">
                                <AddBundle
